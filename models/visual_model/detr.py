...
class DETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_queries, train_backbone, train_transformer, aux_loss=False, use_dconv=False):
    ...
    def forward(self, samples: NestedTensor):
        ...
        # features: (Batch, C(256), H/32, W/32)
        features, pos = self.backbone(samples)
        src, mask, _ = features[-1].decompose()
        assert mask is not None

        if self.transformer is not None:
            out = self.transformer(self.input_proj(
                src), mask, pos[-1], query_embed=None)
        else:
