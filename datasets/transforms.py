...
class NormalizeAndPad(object):
    def __init__(self, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], size=640, mean_padding=False, aug_translate=False):
        self.mean = mean
        self.std = std
        self.size = size
    ...
    def __call__(self, input_dict):
        ...
        if self.mean_padding:
            mean_value = torch.mean(img, (1, 2))
            #out_img = torch.broadcast_to(mean_v, (self.size, self.size, 3)).permute(2, 0, 1).float()
            out_img = mean_value.reshape((3, 1, 1)).repeat(1, self.size, self.size).float()
        else:
...
class VgtrAugmentation(object):
    """docstring for VgtrAugmentation"""
    ...
    def __call__(self, input_dict):
        ...
        input_dict['box'] = torch.tensor(bbox).float()
        return input_dict
