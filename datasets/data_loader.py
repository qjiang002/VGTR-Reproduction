...
class TransVGDataset(data.Dataset):
    ...
    def __init__(self, data_root, split_root='data', dataset='referit', 
                 ...
                 bert_model='bert-base-uncased', embedding_file=None, embedding_dim=300, dataset_fraction=1):
        # self.images is a list of tuples (image file name, bounding box coordinates, query phrase)
        # An example element: ('COCO_train2014_000000014502.jpg', [378.92, 127.98, 183.58, 351.66], 'black shorts guy'
        self.images = []
        ...
        for split in splits:
            ...
            if split == 'train': # truncate the training data for faster training if specified in args
                if self.dataset_fraction <= 0:
                    raise ValueError('Invalid fraction number')
                if self.dataset_fraction < 1:
                    newlen = int(len(self.images) * dataset_fraction)
                    self.images = self.images[:newlen]


        # Load embeddings if LSTMs are used
        if self.lstm:
            self.corpus = Corpus()
            self.corpus.build_dict([self.pull_item(idx, text_only=True) for idx in range(len(self.images))])
            self.corpus.build_embedding(embedding_file, self.corpus.dictionary, self.embedding_dim)
        else:
            self.corpus = None
    ...
    def pull_item(self, idx, text_only=False):
